...
class Token:
    ...
    def __str__(self):
        return "({} {})".format(self.features, self.token)
...
class Tree:
    ...
    def merge_adjacent_tokens(self):
        ...
        if not self.is_continuous():
            ...
            if len(children) == 2:
                ...
                new = Tree(children=[self.children.pop(0), self.children.pop(0)], label='NULL')
                while len(self.children) > 1:
                    new = Tree(children=[new, self.children.pop(0)], label='NULL')
    ...
    def get_point_action(self, cursor_position):
        if self.is_continuous():
            ...
            # 向右的pointing
            if not has_nonleaf:
                cursor_position[self.span_sorted[0]].append([ self.span_sorted[-1]+1, self.label, 0, self.span_sorted, self.get_boundary() ])
                return
            else:
                #向左的pointing.
                for child in self.children:
                    try:
                        ...
                        print("????")
                        pdb.set_trace()
                cursor_position[self.span_sorted[-1]+1].append([self.span_sorted[0], self.label, 0, self.span_sorted, self.get_boundary()])
        #discontinuous
        else:
            try:
            ...
            cursor_position[big].append([small, self.label, 1, self.span_sorted, self.get_boundary() ])
