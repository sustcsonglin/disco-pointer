...
import numpy as np
from src.model.disco_trees import Token, Tree
...
class PointerNet(nn.Module):
    def __init__(self, conf, fields):
        ...
        nn.Linear(2*output_size+self.conf.n_lstm_hidden, self.conf.label_emb_size),
    ...
    def decode(self, x, y):
        ...
        for b_idx in range(batch_size):
            ...
            for i in range(span_num):
                ...
                if lab != '<disco>':
                    #ordinary continuous span.
                    if len(unfinished_discontinuous_span) == 0:
                        add_span = True
                        ...
                        add_span = True
                        unfinished_discontinuous_span.append([l, r])
                        final_r = r

                        prev_r = unfinished_discontinuous_span[0][1]
                        for i in range(1, len(unfinished_discontinuous_span)):
                            if unfinished_discontinuous_span[i][0] < prev_r:
                                add_span = False
                            prev_r = unfinished_discontinuous_span[i][1]

                        if add_span:
                            for uds in unfinished_discontinuous_span:
                                l, r = uds
                                for j in range(l + 1, r):
                                    # overlap detection.
                                    if lookup[j] is not None and lookup[j] < l:
            ...
            # TODO: FIXTHIS.
            try:
                results.append(str(final_node))
            except:
                results.append(str(
                    Tree('TOP',  [Token(raw_word[b_idx][i], i, raw_pos[b_idx][i]) for i in range(sent_len)]
                )
                ))
                print("???????")


            co.append(final_node.cont())
            disco.append(final_node.dis())
            assert len(final_node.span_sorted) == sent_len
        # pdb.set_trace()


        ctx['pred_tree'] = results
        ctx['pred_c'] = co
        ctx['pred_d'] = disco
